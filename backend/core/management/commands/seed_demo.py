...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--reset",
            action="store_true",
            help="Remove the existing KEY demo school dataset before recreating it.",
        )

    def handle(self, *args, **options):
        # Keep each seed phase independent. In particular, do not wrap the whole
        # command in one long-lived transaction because PostgreSQL can retain
        # locks while dependent records are being created.
        if options["reset"]:
        ...
        self.seed_timetables_and_learning_data(
            school,
            academic_year,
            terms,
            classrooms,
        ...
        for key, label in [
            ...
            self.stdout.write(
                f"  {label}: teacher.{key}@key-demo.test / {PASSWORDS['teacher']}"
            )
        self.stdout.write(
            "  Students: student01@key-demo.test … student16@key-demo.test / "
            + PASSWORDS["student"]
        )

    def seed_school(self):
        school, _ = School.objects.update_or_create(
            short_name=DEMO_SHORT_NAME,
            defaults={
                "name": DEMO_SCHOOL,
                "email": "admin@key-demo.test",
                "phone_number": "+254700000001",
                "website": "https://demo.key.test",
    ...
    def seed_academics(self, school):
        return AcademicYear.objects.update_or_create(
            school=school,
            name="2026",
            defaults={
    ...
    def seed_terms(self, year):
        specs = [
            (1, date(2026, 1, 5), date(2026, 4, 3), False),
            (2, date(2026, 5, 4), date(2026, 8, 21), False),
            (3, date(2026, 9, 1), date(2026, 12, 11), True),
        ]
        return [
            ...
            for n, start, end, current in specs
        ]
    ...
    def seed_departments(self, school):
        specs = [
            ...
            (
                "Science & STEM",
                "STEM",
                "Science, computing and practical STEM learning",
            ),
            ("Languages", "LANG", "English language and literacy"),
            (
                "Early Years & Montessori",
                "EY",
                "Early years and Montessori learning",
            ),
        ]
        return {
    ...
    def seed_admin(self, school):
        user = self.user(
            "admin@key-demo.test", "Grace", "Wanjiku", "+254700000010"
        )
        user.is_staff = True
    ...
    def seed_teachers(self, school, departments):
        specs = [
            ("math", "Daniel", "Mwangi", "MATH", "T-MATH-001"),
            ("science", "Aisha", "Otieno", "STEM", "T-STEM-002"),
            ("english", "Peter", "Kamau", "LANG", "T-LANG-003"),
            ("montessori", "Lucy", "Njeri", "EY", "T-EY-004"),
        ]
        teachers = {}
        for key, first, last, department, employee_number in specs:
            user = self.user(
                f"teacher.{key}{DEMO_DOMAIN}",
                first,
                last,
                f"+2547110000{10 + len(teachers)}",
            )
    ...
    def seed_curriculum(self):
        curriculum, _ = Curriculum.objects.update_or_create(
            name="KEY Demo Curriculum",
            defaults={
                "description": "Test curriculum for role isolation and learning workflows.",
                "version": "2026.1",
                "is_active": True,
            },
        )
        ...
        stages = {
            number: CambridgeStage.objects.update_or_create(
                programme=programme,
                stage_number=number,
                defaults={
                    "name": name,
            ...
            for i, (name, number) in enumerate(stage_specs)
        }
        subject_specs = [
            ...
            ("Science", "SCI", True),
            ("Computing", "COMP", True),
            ("Global Perspectives", "GP", False),
            ("Art & Design", "ART", False),
            ("Physical Education", "PE", False),
            ("Montessori Practical Life", "MPL", True),
        ]
        subjects = {
            code: Subject.objects.update_or_create(
                curriculum=curriculum,
                code=code,
                defaults={
                    "name": name,
                    "description": f"{name} demo subject",
                    "is_core": core,
                    "display_order": i + 1,
                    "is_active": True,
                },
            )[0]
            for i, (name, code, core) in enumerate(subject_specs)
        }
        for stage_number, stage in stages.items():
            selected = (
                ["MATH", "ENG", "SCI", "COMP", "PE"]
                if stage_number
                else ["ENG", "MPL", "MATH", "ART"]
            )
            for order, code in enumerate(selected, 1):
                StageSubject.objects.update_or_create(
                    ...
                    defaults={
                        "weekly_lessons": 5 if code in {"MATH", "ENG"} else 3,
                        "is_core": subjects[code].is_core,
                        "display_order": order,
                        "is_active": True,
                    },
                )
    ...
    def seed_classrooms(self, school, year, terms, stages):
        ...
        for term in terms:
            for name, code, stage_number in names:
                classrooms[(term.term_number, code)] = Classroom.objects.update_or_create(
                    school=school,
                    academic_year=year,
                    code=f"{code}-2026-T{term.term_number}",
                    defaults={
                        "term": term,
                        "cambridge_stage": stages[stage_number],
                        "name": name,
                        "capacity": 24 if stage_number == 0 else 30,
                        "is_active": True,
                    },
                )[0]
        return classrooms
    ...
    def seed_teacher_assignments(self, teachers, classrooms, year, terms, subjects):
        mapping = {
            "math": [("G1", "MATH"), ("G3", "MATH")],
            "science": [("G3", "SCI"), ("G5", "SCI"), ("G5", "COMP")],
    ...
    def seed_students(self, school):
        names = [
            ("Amina", "Otieno", Gender.FEMALE),
            ("Brian", "Kamau", Gender.MALE),
            ("Chloe", "Wambui", Gender.FEMALE),
            ("David", "Kiptoo", Gender.MALE),
            ("Ethan", "Mwangi", Gender.MALE),
            ("Faith", "Njeri", Gender.FEMALE),
            ("Grace", "Achieng", Gender.FEMALE),
            ("Hassan", "Ali", Gender.MALE),
            ("Ivy", "Moraa", Gender.FEMALE),
            ("John", "Ochieng", Gender.MALE),
            ("Kevin", "Maina", Gender.MALE),
            ("Lydia", "Atieno", Gender.FEMALE),
            ("Mercy", "Nyambura", Gender.FEMALE),
            ("Noah", "Omondi", Gender.MALE),
            ("Olivia", "Kendi", Gender.FEMALE),
            ("Paul", "Kariuki", Gender.MALE),
        ]
        students = []
        for index, (first, last, gender) in enumerate(names, 1):
            user = self.user(
                f"student{index:02d}{DEMO_DOMAIN}",
                first,
                last,
                f"+254720100{index:02d}",
            )
            ...
            student, _ = Student.objects.update_or_create(
                ...
                defaults={
                    "school": school,
                    "admission_number": f"DEMO-{index:03d}",
                    "admission_date": date(2024, 1, 8),
                    "date_of_birth": date(
                        2015 if index <= 12 else 2016,
                        (index % 12) + 1,
                        (index % 20) + 1,
                    ),
                    "gender": gender,
                    "nationality": "Kenyan",
                    "birth_certificate_number": f"DEMOBC{index:05d}",
                    "is_active": True,
                },
            )
            ...
            for index, student in enumerate(students):
                ...
                enrollment, _ = Enrollment.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        "remarks": "Demo enrollment for end-to-end testing.",
                    },
                )
    ...
    def seed_periods(self, school):
        specs = [
            ...
            (5, "Period 4", time(10, 45), time(11, 30), False),
        ]
        return [
            ...
            for sequence, name, start, end, is_break in specs
        ]

    def seed_timetables_and_learning_data(
        self, school, year, terms, classrooms, teachers, subjects, enrollments, periods
    ):
        teacher_subjects = list(
            TeacherSubject.objects.filter(academic_year=year).select_related(
                "teacher", "subject", "classroom", "term"
            )
        )
        for term in terms:
            timetable, _ = Timetable.objects.update_or_create(
                ...
                version=1,
                defaults={
                    "name": f"Demo Timetable Term {term.term_number}",
                    "status": TimetableStatus.PUBLISHED
                    if term.term_number == 3
                    else TimetableStatus.ARCHIVED,
            ...
            for idx, assignment in enumerate(
                [x for x in teacher_subjects if x.term_id == term.id][:12]
            ):
            ...
            for idx, entry in enumerate(entries):
                session_date = term.start_date + timedelta(
                    days=min(idx + 7, max(0, (term.end_date - term.start_date).days))
                )
                session, _ = LessonSession.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        "started_at": timezone.make_aware(
                            datetime.combine(session_date, time(8, 0))
                        )
                        if idx < 6
                        else None,
                        "ended_at": timezone.make_aware(
                            datetime.combine(session_date, time(8, 45))
                        )
                        if idx < 6
                        else None,
                        "remarks": "Completed demo lesson."
                        if idx < 6
                        else "Upcoming demo lesson.",
                    },
                )
                self.seed_attendance(session, term, enrollments)
                self.seed_assessments(
                    session,
                    entry.teacher_subject.teacher,
                    entry.teacher_subject.subject,
                    term,
                    enrollments,
                )
    ...
    def seed_attendance(self, session, term, enrollments):
        ...
        class_enrollments = [
            e
            for (term_number, _), e in enrollments.items()
            if term_number == term.term_number
            and e.classroom_id == session.timetable_entry.classroom_id
        ]
        statuses = [
    ...
    def _upsert_submission(self, assessment, enrollment, defaults):
        """Upsert without update_or_create's nested savepoint.

        The demo database was stalling while PostgreSQL committed the savepoint
        created by update_or_create on AssessmentSubmission. Since this command
        is normally run with --reset, a direct lookup/create avoids that nested
        transaction path while remaining idempotent when --reset is omitted.
        """
    ...
    def _upsert_evaluation(self, submission, defaults):
        evaluation = AssessmentEvaluation.objects.filter(submission=submission).first()
        ...
        evaluation.save(update_fields=list(defaults.keys()))
        return evaluation
    ...
    def seed_assessments(self, session, teacher, subject, term, enrollments):
        ...
        specs = [
            ...
            (
                ...
                AssessmentStatus.DRAFT if is_current else AssessmentStatus.CLOSED,
                50,
            ),
        ]
        class_enrollments = [
        ...
        for title, assessment_type, status, maximum in specs:
            assessment, _ = Assessment.objects.update_or_create(
                ...
                defaults={
                    "teacher": teacher,
                    "description": f"{title} for {subject.name}. Designed to exercise the student submission and teacher grading workflow.",
                    "assessment_type": assessment_type,
                    "status": status,
                    "due_date": session.lesson_date + timedelta(days=7),
                    "maximum_score": Decimal(str(maximum)),
                ...
                for index, enrollment in enumerate(class_enrollments):
                    ...
                    submission = self._upsert_submission(
                        ...
                        enrollment,
                        {
                            ...
                            "submission_text": f"Demo response from {enrollment.student.user.full_name}.",
                            ...
                            "is_late": index == 2,
                            "teacher_notes": "Good work. Review the feedback."
                            if graded
                            else "",
                    ...
                    if graded:
                        ...
                        self._upsert_evaluation(
                            ...
                            {
                                "total_score": score,
                                "percentage": (score / Decimal(maximum)) * Decimal(100),
                                "narrative_feedback": "Strong understanding demonstrated. Keep improving the explanation of your reasoning.",
                                "published": True,
    ...
    def reset_demo_data(self):
        ...
        AssessmentEvaluation.objects.filter(
            submission__enrollment__student__in=demo_students
        ).delete()
        AssessmentSubmission.objects.filter(
            enrollment__student__in=demo_students
        ).delete()
        Assessment.objects.filter(
            lesson_session__timetable_entry__classroom__school=demo_school
        ).delete()
        ...
        LessonSession.objects.filter(
            timetable_entry__classroom__school=demo_school
        ).delete()
