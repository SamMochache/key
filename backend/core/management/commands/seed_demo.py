...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("\nRun again with --reset to replace only this demo dataset.\n")
    ...
    def seed_terms(self, year):
        ...
        return [
            Term.objects.update_or_create(
                academic_year=year,
                term_number=number,
                defaults={"start_date": start, "end_date": end, "is_current": current, "is_active": True},
            )[0]
            for number, start, end, current in specs
        ]

    def seed_departments(self, school):
        specs = [
            ("Mathematics", "MATH", "Mathematics and numeracy"),
            ("Science & STEM", "STEM", "Science, computing and practical STEM learning"),
            ("Languages", "LANG", "English language and literacy"),
            ("Early Years & Montessori", "EY", "Early years and Montessori learning"),
        ]
        return {
    ...
    def seed_teachers(self, school, departments):
        ...
        for key, first, last, department, employee_number in specs:
            email = f"teacher.{key}{DEMO_DOMAIN}"
            user = self.user(email, first, last, f"+2547110000{10 + len(teachers)}")
    ...
    def seed_curriculum(self):
        curriculum, _ = Curriculum.objects.update_or_create(
            ...
            defaults={"description": "Test curriculum for role isolation and learning workflows.", "version": "2026.1", "is_active": True},
        )
        programme, _ = Programme.objects.update_or_create(
            curriculum=curriculum,
            name="Cambridge Primary",
            defaults={"description": "Primary programme used by the demo school.", "display_order": 1, "is_active": True},
        )
        stage_specs = [
            ("Early Years", 0),
            ("Grade 1", 1),
        ...
        stages = {
            number: CambridgeStage.objects.update_or_create(
                programme=programme,
                stage_number=number,
                defaults={"name": name, "display_order": index + 1, "is_active": True},
            )[0]
            for index, (name, number) in enumerate(stage_specs)
        }
        subject_specs = [
        ...
        subjects = {
            code: Subject.objects.update_or_create(
                curriculum=curriculum,
                code=code,
                defaults={"name": name, "description": f"{name} demo subject", "is_core": core, "display_order": i + 1, "is_active": True},
            )[0]
        ...
        for stage_number, stage in stages.items():
            ...
            for order, code in enumerate(selected, 1):
                StageSubject.objects.update_or_create(
                    ...
                    defaults={"weekly_lessons": 5 if code in {"MATH", "ENG"} else 3, "is_core": subjects[code].is_core, "display_order": order, "is_active": True},
                )
    ...
    def seed_students(self, school):
        names = [
            ("Amina", "Otieno", Gender.FEMALE), ("Brian", "Kamau", Gender.MALE),
            ("Chloe", "Wambui", Gender.FEMALE), ("David", "Kiptoo", Gender.MALE),
            ("Ethan", "Mwangi", Gender.MALE), ("Faith", "Njeri", Gender.FEMALE),
            ("Grace", "Achieng", Gender.FEMALE), ("Hassan", "Ali", Gender.MALE),
            ("Ivy", "Moraa", Gender.FEMALE), ("John", "Ochieng", Gender.MALE),
            ("Kevin", "Maina", Gender.MALE), ("Lydia", "Atieno", Gender.FEMALE),
            ("Mercy", "Nyambura", Gender.FEMALE), ("Noah", "Omondi", Gender.MALE),
            ("Olivia", "Kendi", Gender.FEMALE), ("Paul", "Kariuki", Gender.MALE),
        ]
        students = []
        for index, (first, last, gender) in enumerate(names, 1):
            ...
            student, _ = Student.objects.update_or_create(
                ...
                defaults={
                    ...
                    "date_of_birth": date(2015 if index <= 12 else 2016, (index % 12) + 1, (index % 20) + 1),
                    "gender": gender,
                    "nationality": "Kenyan",
    ...
    def seed_timetables_and_learning_data(self, school, year, terms, classrooms, teachers, subjects, enrollments, periods):
        ...
        for term in terms:
            timetable, _ = Timetable.objects.update_or_create(
                ...
                term=term,
                version=1,
                defaults={"name": f"Demo Timetable Term {term.term_number}", "status": TimetableStatus.PUBLISHED if term.term_number == 3 else TimetableStatus.ARCHIVED, "effective_from": term.start_date, "effective_to": term.end_date},
            )
            entries = []
            for idx, assignment in enumerate([x for x in teacher_subjects if x.term_id == term.id][:12]):
                entry, _ = TimetableEntry.objects.update_or_create(
                    timetable=timetable,
                    weekday=[WeekDay.MONDAY, WeekDay.TUESDAY, WeekDay.WEDNESDAY, WeekDay.THURSDAY, WeekDay.FRIDAY][idx % 5],
                    period=periods[idx % len(periods)],
                    classroom=assignment.classroom,
                    defaults={"teacher_subject": assignment, "room": f"Room {101 + idx}"},
                )
            ...
            for idx, entry in enumerate(entries):
                ...
                session, _ = LessonSession.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        "status": LessonStatus.COMPLETED if idx < 6 else LessonStatus.SCHEDULED,
                        "started_at": timezone.make_aware(datetime.combine(session_date, time(8, 0))) if idx < 6 else None,
                        "ended_at": timezone.make_aware(datetime.combine(session_date, time(8, 45))) if idx < 6 else None,
                        "remarks": "Completed demo lesson." if idx < 6 else "Upcoming demo lesson.",
                    },
                )
    ...
    def seed_attendance(self, session, term, enrollments):
        register, _ = AttendanceRegister.objects.update_or_create(
            ...
            defaults={
                "status": RegisterStatus.SUBMITTED if session.status == LessonStatus.COMPLETED else RegisterStatus.DRAFT,
                "submitted_at": timezone.now() if session.status == LessonStatus.COMPLETED else None,
            },
        )
    ...
    def seed_assessments(self, session, teacher, subject, term, enrollments):
        ...
        specs = [
            ("Weekly Practice", AssessmentType.ASSIGNMENT, AssessmentStatus.PUBLISHED, 20),
            ("Concept Check Quiz", AssessmentType.QUIZ, AssessmentStatus.PUBLISHED, 10),
            ("Practical Project", AssessmentType.PROJECT, AssessmentStatus.DRAFT if is_current else AssessmentStatus.CLOSED, 50),
        ]
        class_enrollments = [e for (term_number, _), e in enrollments.items() if term_number == term.term_number and e.classroom_id == session.timetable_entry.classroom_id]
                ...
                for index, enrollment in enumerate(class_enrollments):
                    ...
                    submission, _ = AssessmentSubmission.objects.update_or_create(
                        ...
                        defaults={
                            ...
                            "status": SubmissionStatus.GRADED if graded else (SubmissionStatus.SUBMITTED if submitted else SubmissionStatus.DRAFT),
                            "is_late": index == 2,
                            "teacher_notes": "Good work. Review the feedback." if graded else "",
