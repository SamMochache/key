...
class StudentSerializer(serializers.ModelSerializer):
    ...
    def get_age(self, obj):
        ...
        birthday = obj.date_of_birth.replace(year=today.year)
    ...
    def validate(self, attrs):
        ...
        if school is not None and request is not None:
            ...
            if not (user.is_staff or user.is_superuser):
                ...
                user_school = getattr(teacher_profile or student_profile, "school", None)
                if user_school is None or user_school.pk != school.pk:
                    raise serializers.ValidationError(
                        {"school": "You can only use your own institution."}
                    )
