...
class ClassReportView(views.APIView):
    ...
    def get(self, request):
        ...
        enrollments = Enrollment.objects.filter(classroom=classroom, academic_year=classroom.academic_year, term=classroom.term).select_related("student", "student__user").order_by("student__admission_number")
        evaluation_qs = AssessmentEvaluation.objects.filter(published=True, submission__enrollment__classroom=classroom, submission__enrollment__academic_year=classroom.academic_year, submission__enrollment__term=classroom.term)
        rows, class_scores, class_attendance_total, class_attendance_present = [], [], 0, 0
        for enrollment in enrollments:
            scores = [float(value) for value in evaluation_qs.filter(submission__enrollment=enrollment).values_list("percentage", flat=True) if value is not None]
            average = round(sum(scores) / len(scores), 1) if scores else None
            attendance = AttendanceRecord.objects.filter(enrollment=enrollment)
            total, present = attendance.count(), attendance.filter(status__in=["PRESENT", "LATE"]).count()
            ...
            rows.append([enrollment.student.admission_number, str(enrollment.student), f"{average:.1f}%" if average is not None else "—", f"{attendance_rate:.1f}%" if attendance_rate is not None else "—", str(len(scores))])
        ...
        story.extend([data_table(table_data, [35 * mm, 80 * mm, 45 * mm, 40 * mm, 30 * mm], center_from=2), Spacer(1, 6 * mm), Paragraph("This report contains published assessment and attendance records available in KEY for the selected class, academic year, and term.", styles["small"])])
        doc.build(story, onFirstPage=footer, onLaterPages=footer)
        buffer.seek(0)
        return FileResponse(buffer, as_attachment=True, filename=f"class-report-{classroom.code}-{classroom.academic_year.name}-term-{classroom.term.term_number}.pdf", content_type="application/pdf")
