...
export function StudentAIReports() {
  ...
  useEffect(() => {
    listPublishedAINarrativeReports()
      ...
      .catch((err) => setError(err instanceof Error ? err.message : 'Unable to load your published reports.'))
  ...
  return (
    <div>
      <PageHeader
        ...
        description="View your published learning progress narratives and download official copies."
      />
      ...
      {!loading && reports.length === 0 && <Card><EmptyState icon="FileText" title="No published reports yet" description="Your teacher or school administrator will publish your learning report when it has been reviewed." /></Card>}
        ...
        {reports.map((report) => (
          <Card key={report.id} className="overflow-hidden">
            <div className="bg-brand-600 text-white px-6 py-5 flex flex-wrap items-center justify-between gap-4">
              <div>
                <p className="font-display font-extrabold text-xl">Learning Progress Report</p>
                <p className="text-brand-100 text-sm mt-1">{report.academic_year_name} · Term {report.term_number} · {report.facts.learner.class}</p>
              </div>
