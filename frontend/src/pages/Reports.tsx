...
import { downloadAssessmentResultsReport, downloadAttendanceReport, downloadClassReport, downloadCompetencyOutcomesReport, downloadStudentReport } from '../lib/reportsApi';
...
export function Reports() {
  ...
  const generateCompetencyReport = async () => {
    ...
    try {
      await downloadCompetencyOutcomesReport({
        classroom: competencyClassroom || undefined,
        student: competencyStudent || undefined,
        academicYear: year || undefined,
        term: term || undefined,
      });
    } catch (err) {
      setReportError(err instanceof Error ? err.message : 'Unable to generate the competency outcomes report.');
    } finally {
  ...
  return (
    <div>
      ...
      {role !== 'student' && <div id="competency-outcomes-report" className="mb-8"><Card className="p-5"><div className="flex flex-col gap-1 mb-5"><h2 className="font-display font-bold text-lg text-slate-800 dark:text-slate-100">Competency Outcomes Report</h2><p className="text-sm text-slate-500 dark:text-slate-400">Generate published competency mastery levels for a class or individual learner.</p></div><div className="grid grid-cols-1 md:grid-cols-4 gap-4"><label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Class<select value={competencyClassroom} onChange={(event) => { setCompetencyClassroom(event.target.value); setCompetencyStudent(''); }} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 font-normal dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"><option value="">All classes</option>{classrooms.map((item) => <option key={item.id} value={item.id}>{item.name} — {item.code}</option>)}</select></label><label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Student (optional)<select value={competencyStudent} onChange={(event) => setCompetencyStudent(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 font-normal dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"><option value="">All students</option>{students.map((item) => <option key={item.id} value={item.id}>{item.full_name} — {item.admission_number}</option>)}</select></label><label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Academic Year<select value={year} onChange={(event) => setYear(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 font-normal dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"><option value="">Any available year</option>{years.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Term<select value={term} onChange={(event) => setTerm(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 font-normal dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"><option value="">Any term</option>{terms.map((item) => <option key={item.id} value={item.id}>Term {item.term_number}</option>)}</select></label></div>{(error || reportError) && <p className="mt-4 text-sm text-red-600 dark:text-red-400">{reportError || error}</p>}<div className="mt-5 flex justify-end"><button type="button" disabled={loading || generatingCompetency || (!competencyClassroom && !competencyStudent)} onClick={generateCompetencyReport} className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-50"><DownloadIcon className="h-4 w-4" />{generatingCompetency ? 'Generating…' : 'Generate Competency PDF'}</button></div></Card></div>}

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{reports.filter((report) => !['Student Progress Report', 'Class Report', 'Attendance Report', 'Assessment Results Report', 'Competency Outcomes Report'].includes(report.title)).map((r) => { const Icon = r.icon; return <Card key={r.title} className="p-5 flex flex-col hover:-translate-y-0.5 transition-transform"><span className={cn('flex h-12 w-12 items-center justify-center rounded-2xl mb-4', tone[r.tone])}><Icon className="h-5 w-5" /></span><h3 className="font-display font-bold text-slate-800 dark:text-slate-100">{r.title}</h3><p className="text-sm text-slate-500 dark:text-slate-400 mt-1 flex-1">{r.desc}</p><div className="flex items-center gap-2 mt-4"><Link to={r.to} className="inline-flex items-center gap-1.5 text-sm font-bold text-brand-600 hover:gap-2 transition-all">Open <ArrowRightIcon className="h-4 w-4" /></Link></div></Card>; })}</div>
    </div>
  );
}
