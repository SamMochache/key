...
export function Parents() {
  ...
  const save = async () => {
    if (!form.first_name || !form.last_name || !form.email || !form.password || !form.school || !form.student) {
      setError('First name, last name, email, password, school, and student are required.');
      return;
    }
    setSaving(true);
    setError('');
    try {
      ...
      setError(err instanceof Error ? err.message : 'Unable to create the parent account.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div>
      ...
      {showForm && <Card className="p-5 mb-6">
        <div className="flex items-center gap-3 mb-5"><div className="h-10 w-10 rounded-2xl bg-brand-50 dark:bg-brand-500/10 flex items-center justify-center"><UserRoundIcon className="h-5 w-5 text-brand-600" /></div><div><h2 className="font-display font-bold text-slate-800 dark:text-slate-100">Create parent account</h2><p className="text-xs text-slate-400">The account will only see reports for explicitly linked learners.</p></div></div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label="Initial password"><input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /></Field>
        ...
        <div className="flex justify-end gap-2 mt-5"><Button variant="secondary" onClick={() => setShowForm(false)}>Cancel</Button><Button onClick={save} disabled={saving}>{saving ? <Loader2Icon className="h-4 w-4 animate-spin" /> : <CheckCircle2Icon className="h-4 w-4" />}{saving ? 'Creating…' : 'Create account'}</Button></div>
      </Card>}
