    ...
    <div>
      ...
      {loading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((item) => (
            <Card key={item} className="h-56 animate-pulse bg-slate-100 dark:bg-slate-800/60" />
          ))}
        </div>
      )}
      ...
      {!loading && error && (
        <Card className="p-6">
          <p className="text-sm font-semibold text-red-600 dark:text-red-400">Unable to load classes</p>
      ...
      {!loading && !error && classrooms.length === 0 && (
        <Card className="p-8 text-center">
          <p className="font-display font-bold text-slate-800 dark:text-slate-100">No active classes yet</p>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Create a classroom in the academic administration area to see it here.</p>
        </Card>
      )}
              ...
              <Card key={classroom.id} className="overflow-hidden hover:-translate-y-1 transition-transform">
                ...
                <div className="p-5">
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
                    {classroom.academic_year_name} · Term {classroom.term_number}
                  </p>
                  ...
                  <div className="mt-5 flex items-center justify-between rounded-2xl bg-slate-50 dark:bg-slate-800/60 px-3.5 py-2.5">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                      ...
                      {classroom.subject_count} subjects
                    </div>
                  </div>
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div className="rounded-2xl bg-slate-50 dark:bg-slate-800/60 py-2.5">
      <p className="font-display font-extrabold text-slate-800 dark:text-slate-100">{value}</p>
