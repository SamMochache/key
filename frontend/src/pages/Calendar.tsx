...
export function Calendar() {
  ...
  const addEvent = async () => {
    if (!canManage || !year || !form.title || !form.start_at || !form.end_at) return;
  ...
  const removeEvent = async (id: string) => {
    if (!canManage || !window.confirm('Delete this calendar event?')) return;
    setSaving(true); setError('');
  ...
  return <div>
    <PageHeader title="Academic Calendar" description="Exams, activities, meetings, deadlines and holidays — the rhythm of the school year." actions={canManage ? <Button onClick={() => { const start = new Date(month.getFullYear(), month.getMonth(), Math.min(new Date().getDate(), monthEnd.getDate()), 8); const end = new Date(start.getTime() + 60 * 60 * 1000); setForm((current) => ({ ...current, start_at: toInputDateTime(start), end_at: toInputDateTime(end) })); setOpen(true); }}><PlusIcon className="h-4 w-4" /> Add event</Button> : undefined} />
